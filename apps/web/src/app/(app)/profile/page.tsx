...
export default async function ProfilePage(props: { searchParams: Promise<{ error?: string }> }) {
  ...
  const roleLabel = top ? `${top.role} · ${top.entity.code}` : 'Aucun rôle actif';
  const lastLogin = user.lastLoginAt
    ? user.lastLoginAt.toLocaleString('fr-FR', { day: 'numeric', month: 'long', year: 'numeric', hour: '2-digit', minute: '2-digit' })
    : 'Jamais';
  ...
  return (
    <div className="space-y-6">
      ...
      {errorMessage && (
        <div role="alert" className="rounded-[10px] border border-[var(--color-destructive)] bg-[var(--color-destructive-soft)] px-3 py-2 text-sm text-[var(--color-destructive)]">
          {errorMessage}
        </div>
      )}
