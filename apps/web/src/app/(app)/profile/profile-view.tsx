...
export function ProfileView({
  ...
  const [locale, setLocale] = useState(preferredLocale === 'en-US' ? 'en-US' : 'fr-FR');

  const initials = ((firstName[0] ?? '') + (lastName[0] ?? '')).toUpperCase() || name.slice(0, 2).toUpperCase();
  const reset = () => {
    setFn(firstName);
    setLn(lastName);
    setLocale(preferredLocale === 'en-US' ? 'en-US' : 'fr-FR');
  };
  ...
  const fieldLabel = 'mb-1.5 block text-[11px] font-semibold uppercase tracking-[0.1em] text-[var(--color-faint)]';
  const inputCls =
    ...
    <div className="grid gap-6 lg:grid-cols-[300px_1fr]">
      ...
      <aside className="h-fit rounded-[14px] border border-[var(--color-border)] bg-[var(--color-card)] p-6 shadow-[var(--shadow-sm)]">
        <div className="flex flex-col items-center text-center">
          ...
          <div className="mt-4 text-[17px] font-semibold text-[var(--color-foreground)]">{name}</div>
          <div className="text-[13px] text-[var(--color-faint)]">{email}</div>
          <div className="mt-3 text-[11px] font-semibold uppercase tracking-[0.14em] text-[var(--color-primary)]">{roleLabel}</div>
        </div>
        ...
        <div className="space-y-4 text-[13px]">
          <div>
            <div className="text-[11px] font-semibold uppercase tracking-[0.1em] text-[var(--color-faint)]">Dernière connexion</div>
            <div className="mt-1 text-[var(--color-foreground)]">{lastLogin}</div>
          </div>
          <div className="flex items-center justify-between">
            <div className="text-[11px] font-semibold uppercase tracking-[0.1em] text-[var(--color-faint)]">Statut</div>
            <span
              className={
      ...
      <section className="rounded-[14px] border border-[var(--color-border)] bg-[var(--color-card)] shadow-[var(--shadow-sm)]">
        <div className="flex gap-1 border-b border-[var(--color-border)] px-4">
          {tabBtn('infos', 'Informations', <svg viewBox="0 0 24 24" {...s}><circle cx="12" cy="8" r="4" /><path d="M4 21c0-4 3.5-6 8-6s8 2 8 6" /></svg>)}
          {tabBtn('securite', 'Sécurité', <svg viewBox="0 0 24 24" {...s}><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z" /></svg>)}
          {tabBtn('prefs', 'Préférences', <svg viewBox="0 0 24 24" {...s}><line x1="4" y1="21" x2="4" y2="14" /><line x1="4" y1="10" x2="4" y2="3" /><line x1="12" y1="21" x2="12" y2="12" /><line x1="12" y1="8" x2="12" y2="3" /><line x1="20" y1="21" x2="20" y2="16" /><line x1="20" y1="12" x2="20" y2="3" /><line x1="1" y1="14" x2="7" y2="14" /><line x1="9" y1="8" x2="15" y2="8" /><line x1="17" y1="16" x2="23" y2="16" /></svg>)}
        </div>
        ...
        <form action={updateAction} className="p-6">
          ...
          <div className={tab === 'infos' ? 'space-y-5' : 'hidden'}>
            ...
            <div>
              ...
              <input value={email} readOnly className={inputCls + ' bg-[var(--color-surface-2)] text-[var(--color-muted-foreground)]'} />
              <p className="mt-1.5 text-[12px] text-[var(--color-faint)]">Cet email sert à vous identifier sur la plateforme.</p>
            </div>
          </div>
          ...
          <div className={tab === 'securite' ? 'space-y-4' : 'hidden'}>
            <div>
              ...
              >
                <svg viewBox="0 0 24 24" className="h-4 w-4" {...s}><circle cx="8" cy="15" r="4" /><path d="M10.8 12.2 20 3M16 6l3 3M14 8l2 2" /></svg>
                Réinitialiser le mot de passe
              </a>
            </div>
            <p className="text-[12px] text-[var(--color-faint)]">Dernière connexion : {lastLogin}.</p>
          </div>

          {/* PRÉFÉRENCES */}
          <div className={tab === 'prefs' ? 'space-y-6' : 'hidden'}>
            <div>
              <label className={fieldLabel}>Langue de l&apos;interface</label>
              <div className="inline-flex rounded-[12px] border border-[var(--color-border)] bg-[var(--color-surface-2)] p-1">
                {[
                  { v: 'fr-FR', code: 'FR', label: 'Français', on: true },
                  { v: 'en-US', code: 'EN', label: 'English', on: true },
                  { v: 'zh-CN', code: 'CN', label: '中文', on: false },
                ].map((o) => (
                  <button
                    key={o.v}
                    type="button"
                    disabled={!o.on}
                    onClick={() => o.on && setLocale(o.v)}
                    title={o.on ? '' : 'Bientôt (i18n)'}
                    className={
                  ...
                  >
                    <span className="text-[10px] font-semibold text-[var(--color-faint)]">{o.code}</span>
                    {o.label}
                  </button>
                ))}
              </div>
              <p className="mt-1.5 text-[12px] text-[var(--color-faint)]">La langue choisie est mémorisée pour les prochaines connexions.</p>
            </div>
            <div className="max-w-sm">
              <label className={fieldLabel}>Fuseau horaire d&apos;affichage</label>
              <select name="preferredTimezone" defaultValue={preferredTimezone} className={inputCls}>
                {timezones.map((tz) => (
        ...
        <div className="border-t border-[var(--color-border)] px-6 py-5">
          <div className={fieldLabel}>Session</div>
          <p className="mb-3 text-[12px] text-[var(--color-faint)]">En cas de doute sur la sécurité de votre session, déconnectez-vous puis reconnectez-vous.</p>
          <form action={logoutAction}>
            ...
            >
              <svg viewBox="0 0 24 24" className="h-4 w-4" {...s}><path d="M9 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h4" /><polyline points="16 17 21 12 16 7" /><line x1="21" y1="12" x2="9" y2="12" /></svg>
              Déconnexion
            </button>
          </form>
        </div>
