...
export default async function JournalEntryDetailPage(props: {
  ...
  async function handlePost(formData: FormData) {
    ...
    if (!r.ok) redirect('/accounting/entries/' + id + '?error=' + encodeURIComponent(r.error ?? 'Echec'));
  }
  ...
  return (
    <div className="space-y-6">
      <header className="flex items-baseline justify-between">
        <div>
          ...
          <p className="text-sm text-[var(--color-muted-foreground)]">
            {entry.description} - Journal {entry.journalCode}
          </p>
        </div>
      ...
      <section className="grid grid-cols-1 gap-4 sm:grid-cols-4">
        ...
        <div className="rounded-lg border bg-[var(--color-card)] p-4 shadow-sm">
          <div className="text-xs uppercase text-[var(--color-muted-foreground)]">Periode</div>
          <div className={'mt-1 font-mono text-sm ' + (entry.period.isClosed ? 'text-[var(--color-destructive)]' : 'text-[var(--color-success)]')}>
            {entry.period.year}-{String(entry.period.month).padStart(2, '0')} {entry.period.isClosed ? 'CLOSE' : 'OUVERTE'}
          </div>
        </div>
        <div className="rounded-lg border bg-[var(--color-card)] p-4 shadow-sm">
          <div className="text-xs uppercase text-[var(--color-muted-foreground)]">Total debit</div>
          <div className="mt-1 text-xl font-semibold tabular-nums">
            {formatCurrency(Number(entry.totalDebit.toString()), entry.currency)}
          </div>
        </div>
        <div className="rounded-lg border bg-[var(--color-card)] p-4 shadow-sm">
          <div className="text-xs uppercase text-[var(--color-muted-foreground)]">Equilibre</div>
          <div className={'mt-1 font-mono text-sm ' + (balanced ? 'text-[var(--color-success)]' : 'text-[var(--color-destructive)]')}>
            {balanced ? 'OK' : 'KO'}
          </div>
        </div>
      </section>
      ...
      <section className="rounded-lg border bg-[var(--color-card)] shadow-sm">
        <header className="border-b px-4 py-3">
          <h2 className="text-sm font-semibold">Lignes ({entry.lines.length})</h2>
        </header>
            ...
            {entry.lines.map((l) => (
              <tr key={l.id} className="border-b last:border-0">
                ...
                <td className="px-3 py-2">
                  ...
                  <div className="text-[10px] text-[var(--color-muted-foreground)]">{l.account?.label}</div>
                </td>
                <td className="px-3 py-2 text-xs">{l.description ?? '-'}</td>
                <td className="px-3 py-2 text-right tabular-nums">
                  {Number(l.debit.toString()) > 0 ? formatCurrency(Number(l.debit.toString()), entry.currency) : ''}
                </td>
                <td className="px-3 py-2 text-right tabular-nums">
                  {Number(l.credit.toString()) > 0 ? formatCurrency(Number(l.credit.toString()), entry.currency) : ''}
                </td>
              </tr>
            ))}
            <tr className="border-t-2 font-semibold">
              <td colSpan={3} className="px-3 py-2 text-xs uppercase">Total</td>
              <td className="px-3 py-2 text-right tabular-nums">{formatCurrency(Number(entry.totalDebit.toString()), entry.currency)}</td>
              <td className="px-3 py-2 text-right tabular-nums">{formatCurrency(Number(entry.totalCredit.toString()), entry.currency)}</td>
            </tr>
          </tbody>
        </table>
      </section>
      ...
      <section className="rounded-lg border bg-[var(--color-card)] p-6 shadow-sm">
        ...
        {entry.payment && (
          <p className="mt-3 text-xs text-[var(--color-muted-foreground)]">
            Generee depuis paiement{' '}
            <Link href={'/payments/' + entry.payment.id} className="text-[var(--color-primary)] hover:underline font-mono">
